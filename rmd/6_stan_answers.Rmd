...
NOTE: I took a mark off if your file paths pointed to your local machine. 
...
kidiq %>% 
  ...
  ggplot(aes(kid_score, mom_iq, color = mom_hs)) + geom_point()
```
...
kidiq %>% 
  mutate(mom_hs = as_factor(mom_hs)) %>% 
  ggplot(aes(mom_iq, fill = mom_hs)) + geom_histogram(position = 'dodge', aes(y = ..density..))
```
