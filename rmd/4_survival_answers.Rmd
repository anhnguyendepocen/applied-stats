...
Histogram of times by age suggest similar patterns, but many more births to younger women:

```{r}
f12 %>% 
  filter(event==1) %>%  
  ggplot(aes(next.ivl, fill = age_group)) + geom_histogram(position = 'dodge')
```
