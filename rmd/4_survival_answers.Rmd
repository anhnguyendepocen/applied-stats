...
A: Could do a lot of different things here; note that (as usual) I was expecting you to also explain the charts, not just do the charts (i.e. tell me why they are interesting).

Histogram of times by age suggest similar patterns, but young people have a longer tail:

```{r}
f12 %>% 
  ggplot(aes(next.ivl, fill = age_group)) + geom_histogram(position = 'dodge', aes(y = ..density..))
```

Plotting the mean birth interval by year suggests a big outlier at the start of the dataset:

```{r}
f12 %>% 
  group_by(year) %>% 
  summarise(mean_ivl = mean(next.ivl)) %>% 
  ggplot(aes(year, mean_ivl)) + geom_line() + 
  ggtitle("Mean birth interval by year")
```

Not much of a relationship from scatter plot of previous and next birth interval, for short intervals at least:

```{r}
f12 %>% 
  filter(next.ivl<5, prev.ivl<5) %>% 
  ggplot(aes(next.ivl, prev.ivl, color = age_group)) + 
  geom_point() + 
  geom_smooth()
```
...
f12 <- f12 %>% 
  arrange(next.ivl) %>% 
  mutate(
         cumulative_people_lost = 1:n(),
         n_i = lag(n - cumulative_people_lost, default = n),
         prob_death = event/n_i,
...
```{r}
cbind(exp(coef(fit_pois)), D_k/E_k)
```
...
```{r}
fit_age <- glm(event ~ offset(log(interval_length))-1 + interval + age_group, data=f12_split, family = "poisson")
summary(fit_age)
```
...
A: almost 96% of younger women give birth within 5 years. 

```{r}
lambdas_young <- exp(coef(fit_age)[-C+1])
lambdas_old <- exp(coef(fit_age)[-C+1] + coef(fit_age)[C+1])
1- (survival_prob(lambdas_young, cuts) %>% filter(time==5) %>% select(surv) %>% pull()) 
```
